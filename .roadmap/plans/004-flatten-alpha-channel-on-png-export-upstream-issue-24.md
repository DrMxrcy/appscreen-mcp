...
## 🛠️ Checklist
- [x] Step 1: In export path, composite onto an opaque canvas (fill #000/#fff or background color first) before toDataURL/toBlob -> target: app.js exportCurrent/exportAll
- [x] Step 2: Verify: export image with transparent background, inspect pixels (no alpha < 255); batch/ZIP export too -> target: manual + script check
