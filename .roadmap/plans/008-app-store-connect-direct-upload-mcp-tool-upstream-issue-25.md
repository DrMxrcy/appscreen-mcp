...
- [ ] Step 1: Research ASC API screenshot upload flow (screenshot sets, asset reservation, chunked upload, commit + checksum) and pick JWT lib -> target: analysis note
- [ ] Step 2: Implement asc-upload.ts client with dimension/alpha pre-validation -> target: mcp-server/src/asc-upload.ts
- [ ] Step 3: Register `upload_screenshots` tool with env-based credential resolution -> target: mcp-server/src/index.ts
- [ ] Step 4: Verify against a real/test ASC app (dry-run mode that stops before commit); document setup -> target: mcp-server/README.md
